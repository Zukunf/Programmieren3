...
window.addEventListener("DOMContentLoaded", function () {
  const pElement = document.getElementById("Senden");
  if (!pElement) {
    console.error("Element mit ID 'Senden' nicht gefunden.");
    return;
  }
  ...
  pElement.addEventListener("click", function () {
    ...
    for (let row = 0; row < matrix.length; row++) {
      ...
      for (let col = 0; col < matrix[row].length; col++) {
        ...
        newRow.push(cell.constructor.name); 
      }
    ...
    socket.emit("send message", daten);
  });
});
